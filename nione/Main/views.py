...
import random,smtplib,string,datetime
...
def ctom(request):
    ...
    mobn = request.POST['mobn']
    Account(email=email,passw=passw,name=name,mobno=mobn,addr=addr).save()
...
def change(request):
    ...
    fr.save()
    rr = ForgetPass.objects.get(email = request.POST['email'])
    rr.delete()
...
def placeOrder(request):
    ...
    paymentMethod = request.POST['paymentM']
